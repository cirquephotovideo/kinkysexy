...
async function main() {
  ...
  const adminPassword = process.env.ADMIN_PASSWORD || 'KinkySexy2026!';
  ...
  const products = [
    {
      ...
      title: 'T-shirt arc-en-ciel "KinkySexy"',
      description: `T-shirt unisexe en coton bio (180 g/m²), imprimé en France.
Devant : grand cœur arc-en-ciel rainbow shift au feutre néon, signé "KinkySexy".
...
Édition signée et numérotée — la première série est limitée à 200 exemplaires.`,
  ...
  if (ordersCount === 0) {
    const seededProducts = await prisma.product.findMany({ where: { slug: { in: ['t-shirt-arc-en-ciel', 'bougie-vitrail', 'mug-foi-inclusive', 'tote-bag-kinky', 'affiche-cathedrale-lumiere'] } } });
    if (seededProducts.length >= 3) {
      ...
      await prisma.order.create({
        data: {
          ...
          shippingZip: '13002',
          shippingCountry: 'FR',
          totalCents: find('tote-bag-kinky').priceCents + find('mug-foi-inclusive').priceCents * 2,
          currency: 'EUR',
          status: 'PENDING',
          paymentProvider: 'STRIPE',
          weightGrams: 980,
          ...
          items: {
            create: [
              { productId: find('tote-bag-kinky').id, quantity: 1, priceCents: find('tote-bag-kinky').priceCents, variant: { Couleur: 'Naturel' } },
