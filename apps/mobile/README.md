# 📱 KinkySexy — App iOS / iPadOS / Android

App mobile **Expo / React Native / TypeScript** connectée à l'API web sur https://kinky.pixeeplay.com.
...
Bonus :
- **Chat IA "Demandez à Kinky"** (modal plein écran)
...
```
Cela crée le dossier `ios/` avec `KinkySexy.xcodeproj` ouvrable dans Xcode.

### Lancer en simulateur
```bash
npm run ios
```
Ou ouvre `ios/KinkySexy.xcworkspace` dans Xcode et clique ▶.
...
2. Ouvre `ios/KinkySexy.xcworkspace`
...
### Bundle Identifier
Dans `app.json` : `ios.bundleIdentifier`. Actuellement `com.pixeeplay.kinkysexy` — à adapter à ton compte développeur Apple si différent.

### URL de l'API
Par défaut : `https://kinky.pixeeplay.com` (déjà en prod).
...
├── lib/
│   └── api.ts                    # Wrapper fetch vers kinky.pixeeplay.com
...
| Erreur | Solution |
|---|---|
| "No development team selected" | Ouvre `ios/KinkySexy.xcworkspace` → Signing & Capabilities → choisis ton équipe |
...
Pour toute question, l'API web tourne déjà sur https://kinky.pixeeplay.com — tu peux tester avec Postman avant que l'app soit prête.
