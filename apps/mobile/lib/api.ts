...
export const API_BASE: string =
  (Constants.expoConfig?.extra?.apiBase as string) || 'https://kinky.pixeeplay.com';

async function getDeviceToken() {
  try {
    return await SecureStore.getItemAsync('kinky-device-token');
  } catch {
    return null;
  }
}
...
export async function setDeviceToken(token: string) {
  await SecureStore.setItemAsync('kinky-device-token', token);
}
