...
### État actuel
Tout ce qui est "Kinky" est codé en dur dans :
- Textes : composants ont "Kinky", "KinkySexy", "kinky.pixeeplay.com" partout
...
Je peux te coder l'Option 2 en quelques sessions. Ça commencerait par le `branding.config.ts` + un cron de validation qui détecte tout le code restant qui a "Kinky" en dur.
...
### Tâches IA actuellement utilisées sur Kinky (par catégorie)
...
| **"Demandez à Kinky" RAG** | conversation | 1000-3000 t | Gemini + RAG | Ollama (Mac) + ton vector store | `llama3.1:8b` + `nomic-embed-text` pour embeddings |
...
LM Studio expose déjà un serveur compatible OpenAI (`http://localhost:1234/v1/chat/completions`). On peut le configurer comme un provider parmi d'autres dans la nouvelle page `/admin/ai-settings`. Avantage : tu peux **swap** le modèle dans LM Studio GUI sans toucher au code Kinky.
...
| **Plan recommandé Kinky** | Ollama prioritaire, Gemini en fallback grounded uniquement | ~5 €/mois électricité + 0-20 €/mois Gemini selon volume |
