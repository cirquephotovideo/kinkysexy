/**
 * pm-sync — synchronise le catalogue productsmanager.app vers les PriceWatch Kinky.
 *
      ...
      if (watch) {
        ...
        await prisma.priceWatch.update({
          ...
          data: {
            pmProductId: p.id,
            pmSyncedAt: new Date(),
            // On ne touche aux champs canoniques que s'ils sont vides côté Kinky (PM peut être moins précis)
