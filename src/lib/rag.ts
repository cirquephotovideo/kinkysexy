...
const DEFAULT_SYSTEM_PROMPT = `Tu es la voix officielle du mouvement « KinkySexy » (Kinky).
...
Réponds maintenant dans le style Kinky défini plus haut.`;
}
  ...
  if (chunks.length > 0) {
    ...
    prompt = `${sysPrompt}\n\nQUESTION :\n${question}\n\n${
      opts.bypassGuardrails
        ...
        : 'Réponds dans le style Kinky défini plus haut. (Note : la bibliothèque de connaissances RAG n\'est pas encore alimentée, réponds depuis ton savoir général en restant fidèle au ton et aux principes Kinky.)'
    }`;
  }
