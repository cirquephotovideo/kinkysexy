...
const SYSTEM_BRAND = `
Tu es l'assistant éditorial du mouvement "KinkySexy".
Ligne éditoriale :
...
export async function aiCaptionImage(imageUrl: string, words = 80) {
  ...
  return gemini({
    prompt: `Décris cette photo en environ ${words} mots, ton chaleureux et inclusif, en français. Si tu vois un lieu de culte, identifie-le. Termine par 3 hashtags pertinents dont #KinkySexy. Pas d'emoji.`,
    images: [img]
  });
}
...
export async function aiPolishTestimony(rawText: string, anonymize = true) {
  return gemini({
    prompt: `Réécris ce témoignage brut pour publication sur le site KinkySexy.
...
export async function aiNewsletterMonth(stats: any) {
  return gemini({
    prompt: `Rédige une newsletter mensuelle pour le mouvement KinkySexy, en HTML simple (pas de <html>/<body>).
...
export async function aiVerseOfTheDay(theme?: string) {
  return gemini({
    prompt: `Rédige un message inspirant quotidien pour les abonnés de KinkySexy.
${theme ? `Thème : ${theme}` : ''}
...
export async function aiWeeklyDigest(stats: any) {
  return gemini({
    prompt: `Rédige une synthèse hebdomadaire pour l'admin du mouvement KinkySexy.
...
export async function aiInclusiveChat(question: string, history: { role: 'user' | 'model'; text: string }[] = []) {
  ...
  const system = `Tu es "Kinky", l'assistant inclusif du mouvement KinkySexy.
