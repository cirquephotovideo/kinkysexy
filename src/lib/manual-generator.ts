...
const SECTIONS_USER: Section[] = [
  { title: 'Bienvenue', emoji: '🌈', routes: ['/'], hint: 'Page d\'accueil Kinky : hero, témoignages, ticker dons, partenaires, footer' },
  { title: 'Découvrir l\'argumentaire', emoji: '📖', routes: ['/argumentaire', '/message'], hint: 'Pages éditoriales sur la mission Kinky : foi inclusive, message d\'amour' },
  ...
  { title: 'Demandez à Kinky (chat IA)', emoji: '✨', routes: ['/'], hint: 'Widget chat avec avatar IA RAG sur la base de connaissance' },
...
const SECTIONS_SUPERADMIN: Section[] = [
  ...SECTIONS_ADMIN,
  { title: 'AI Autopilot', emoji: '🤖', routes: ['/admin/ai-autopilot'], hint: 'Kinky Soul + Mood + Modération + Newsletter auto, toggles ultra-fins, quota Gemini' },
...
const AUDIENCES: Record<Audience, { sections: Section[]; tone: string; intro: string }> = {
  user: {
    ...
    intro: 'Bienvenue dans KinkySexy ! Ce manuel te guide à travers toutes les fonctionnalités du site pour que tu te sentes chez toi.'
  },
  admin: {
    sections: SECTIONS_ADMIN,
    tone: 'pro, efficace, orienté tâches concrètes. Explique le "comment" et le "pourquoi". Vouvoiement neutre.',
    intro: 'Ce manuel admin couvre toutes les pages du back-office Kinky : modération, contenu, intégrations.'
  },
  superadmin: {
...
export async function generateManual(audience: Audience): Promise<GeneratedManual> {
  ...
  for (const sec of cfg.sections) {
    ...
    if (!quotaExhausted) {
      const prompt = `Tu rédiges UNE section d'un manuel utilisateur pour le site KinkySexy (réseau social inclusif religieux LGBT+, https://kinky.pixeeplay.com).
...
<title>Manuel ${audienceLabels[audience]} — Kinky v${version}</title>
<style>
...
<div class="container">
  ...
  <div class="meta">
    ...
    Régénéré 2×/jour pour rester à jour. — KinkySexy · kinky.pixeeplay.com
  </div>
</div>
</body>
</html>`;
...
function renderMarkdown(audience: Audience, version: string, intro: string, sections: any[]): string {
  ...
  let md = `# 🌈 KinkySexy — Manuel ${labels[audience]}\n\nv${version}\n\n${intro}\n\n## Sommaire\n\n`;
...
function renderVideoScript(audience: Audience, sections: any[]): string {
  const labels: Record<Audience, string> = { user: 'Utilisateur', admin: 'Administrateur', superadmin: 'Super-Admin' };
  let script = `# 🎬 SCRIPT VIDÉO — Manuel ${labels[audience]} Kinky\n\n`;
  script += `**Durée totale estimée :** ${Math.ceil(sections.length * 0.7)} minutes\n\n`;
  script += `---\n\n## 🎬 OUVERTURE (0:00 - 0:15)\n\n`;
  script += `**Voix off :**\n> Bienvenue dans KinkySexy. Voici un tour complet du site.\n\n`;
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (s.routes.length) script += `**Screenshot à capturer :** ${s.routes.map((r: string) => `\`https://kinky.pixeeplay.com${r}\``).join(', ')}\n`;
  ...
  script += `**Voix off :**\n> Pour aller plus loin, rejoins kinky.pixeeplay.com. Dieu est amour. La foi se conjugue au pluriel.\n\n`;
  script += `**Screenshot :** logo Kinky + lien site\n**Outro :** musique fade-out 5 sec\n\n---\n\n`;
  script += `## 📸 LISTE DES SCREENSHOTS À CAPTURER\n\n`;
  const allRoutes = Array.from(new Set(sections.flatMap((s) => s.routes)));
  for (const r of allRoutes) script += `- [ ] \`https://kinky.pixeeplay.com${r}\`\n`;
  return script;
}
