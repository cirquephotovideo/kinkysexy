 ...
 * dans le RAG Kinky pour transformer le chat en assistant juridique français.
 *
...
async function fetchTree(): Promise<GitHubTreeEntry[]> {
  const r = await fetch(TREES_API, {
    headers: { 'User-Agent': 'Kinky-Paperasse-Sync/1.0', Accept: 'application/vnd.github.v3+json' },
...
async function fetchRaw(path: string): Promise<string> {
  const r = await fetch(`${RAW_BASE}/${path}`, {
    headers: { 'User-Agent': 'Kinky-Paperasse-Sync/1.0' },
    signal: AbortSignal.timeout(15_000),
  });
