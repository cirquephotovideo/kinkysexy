/**
 * price-rag-sync — synchronise les PriceWatch dans le RAG « Demandez à Kinky ».
 *
 ...
 *
 * Résultat : le chat Kinky peut répondre à
...
export function watchToMarkdown(watch: any): string {
  ...
  lines.push('');
  lines.push(`Ce document a été généré automatiquement par le comparateur de prix Kinky le ${new Date().toLocaleString('fr-FR')}.`);
  return lines.join('\n');
}
