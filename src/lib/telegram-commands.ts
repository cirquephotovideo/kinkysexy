/**
 * Router de commandes Telegram pour le bot Kinky.
...
async function cmdHelp(chatId: number, userId: number) {
  const text = `<b>🌈 Bot Kinky — 40+ commandes</b>
...
async function cmdPhotos(chatId: number, args: string[]) {
  ...
  for (const p of photos) {
    ...
    try {
      ...
      const origin = process.env.NEXT_PUBLIC_SITE_URL || 'https://kinky.pixeeplay.com';
      ...
      await sendMessage(chatId, `${caption}\n\n<a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://kinky.pixeeplay.com'}/admin/photos/${p.id}">Voir dans l'admin</a>`, { inline_keyboard: buttons });
    }
  }
}
