...
// ─────────────────────────────────────────────
// 1. Catalogue des TÂCHES IA du site Kinky
// ─────────────────────────────────────────────
export const AI_TASKS = {
  ...
  'rag-chat':             { label: '"Demandez à Kinky" RAG chat',              tokens: 3000,   description: 'Chat avec retrieval augmenté sur knowledge base.' },
  ...
  'avatar-realtime':      { label: 'Avatar IA temps-réel',                    tokens: 0,      description: 'Kinky Live — vidéo + voix temps-réel.' }
} as const;
...
async function callOpenAICompat(provider: ProviderConfig, model: string, opts: GenerateOptions, kind: string): Promise<string> {
  ...
  if (kind === 'openrouter') {
    headers['HTTP-Referer'] = 'https://kinky.pixeeplay.com';
    headers['X-Title'] = 'KinkySexy';
  }
  const r = await fetch(url, {
    method: 'POST',
    headers,
