 ...
 *
 * Canaux :
 *   - Email via Resend (déjà configuré dans Kinky : RESEND_API_KEY)
...
async function sendAlert(watch: any, triggers: { type: string; details: string }[], r: RefreshResultMin) {
  ...
  const body = [
    ...
    '',
    `Voir le détail : https://kinky.pixeeplay.com/admin/prices/${watch.id}`,
  ].join('\n');
  ...
  if (email && resendKey) {
    try {
      const resp = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: process.env.EMAIL_FROM || 'Kinky Comparator <noreply@kinky.pixeeplay.com>',
          to: email,
          subject,
          text: body,
        }),
          ...
          blocks: [
            ...
            { type: 'section', text: { type: 'mrkdwn', text: '```' + body + '```' } },
            {
              type: 'actions', elements: [
                { type: 'button', text: { type: 'plain_text', text: 'Voir le détail' }, url: `https://kinky.pixeeplay.com/admin/prices/${watch.id}` },
              ]
            },
          ],
        }),
