/**
 * Bibliothèque de 100 effets CSS purs pour le Page Builder Kinky.
 *
 * Chaque effet a :
 *   - id        : identifiant unique kebab-case
 *   - name      : libellé affiché
 *   - category  : entry | hover | scroll | text | background | card | transition | reveal
 *   - css       : règles CSS à injecter (animation @keyframes + class .kinky-fx-<id>)
 *   - usage     : sur quoi l'appliquer (block | text | image | card | hero)
 *
 * À utiliser via <EffectWrapper effect="zoom-in"> ou class={`kinky-fx-${id}`}.
