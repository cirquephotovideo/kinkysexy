...
/**
 * Construit le prompt complet Kinky à partir du systemPrompt + corpus RAG.
...
export async function buildGldContextPrompt(maxChars: number = 24000): Promise<{ prompt: string; opening: string; docCount: number; chunksUsed: number }> {
  ...
  const sysPrompt = cfg['rag.systemPrompt'] || `Tu es l'assistant officiel du mouvement « KinkySexy » (Kinky).
...
Réponses courtes (3-5 phrases max) et termine par une question d'ouverture.`;
  ...
  const fullPrompt = `${sysPrompt}\n\n# BASE DE CONNAISSANCES Kinky\nVoici les textes officiels du mouvement. Utilise-les en priorité pour répondre. Cite la source quand pertinent.\n${knowledgeBlock || '\n(aucun document encore disponible)'}`;
  const opening = 'Bonjour ! Je suis Kinky, l\'assistant du mouvement. Pose-moi une question sur la foi, l\'inclusion et l\'amour radical.';
...
export async function ensureContext(): Promise<{ contextId: string | null; reason?: string }> {
  ...
  if (!contextId) {
    try {
      ...
      const ctx = await createContext('Kinky - Brain (RAG synced)', built.prompt, built.opening);
      contextId = ctx.id;
...
export async function syncContextWithRag(): Promise<{ ok: boolean; contextId: string; docsIncluded: number; chars: number; reason?: string }> {
  ...
  try {
    if (contextId) {
      await updateContext(contextId, 'Kinky - Brain (RAG synced)', built.prompt, built.opening);
    } else {
      const ctx = await createContext('Kinky - Brain (RAG synced)', built.prompt, built.opening);
      contextId = ctx.id;
      await setSetting('avatar.liveavatar.contextId', contextId);
