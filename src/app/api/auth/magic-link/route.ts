...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const url = `https://kinky.pixeeplay.com/api/auth/magic-link/redeem?token=${token}&email=${encodeURIComponent(email)}`;
    const html = `
      <div style="font-family:system-ui;max-width:600px;margin:0 auto;padding:24px;background:#fff;color:#222">
        <h1 style="color:#d61b80">✨ Lien magique Kinky</h1>
    ...
    `;
    await sendEmail(email, '✨ Ton lien de connexion magique Kinky', html, { type: 'admin-notify' });
