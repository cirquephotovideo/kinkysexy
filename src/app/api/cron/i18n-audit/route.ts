 ...
 *
 * À planifier :
 *  - Coolify scheduled tasks → curl -H "X-Cron-Secret: $SECRET" https://kinky.pixeeplay.com/api/cron/i18n-audit
...
export async function GET(req: NextRequest) {
  ...
  await notify({
    event: 'i18n.audit',
    title: `🌍 ${report.totalIssues} traduction${report.totalIssues > 1 ? 's' : ''} manquante${report.totalIssues > 1 ? 's' : ''} sur Kinky`,
    body: `${summary}\n\n${detail}\n\n→ /admin/i18n pour traduire en 1 clic avec Gemini`,
    url: 'https://kinky.pixeeplay.com/admin/i18n',
    level: 'warning'
  });
