...
export async function POST(req: Request) {
  ...
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://kinky.pixeeplay.com';
  ...
  try {
    const r = await fetch(`${base}/v2/online-checkout/payment-links`, {
      ...
      body: JSON.stringify({
        ...
        quick_pay: {
          name: `Don à KinkySexy — ${amount} ${currency}`,
        ...
        note: note || `Don de soutien au mouvement KinkySexy`
      })
    });
