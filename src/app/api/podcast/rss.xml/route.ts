 ...
 *
 * Pour publier :
 * 1. Spotify for Podcasters → ajouter le podcast par RSS → coller https://kinky.pixeeplay.com/api/podcast/rss.xml
 * 2. Apple Podcasts Connect → idem
 */
export async function GET() {
  const SITE = process.env.NEXT_PUBLIC_SITE_URL || 'https://kinky.pixeeplay.com';
  let testimonies: any[] = [];
  try {
  ...
  const items = testimonies
    ...
    <item>
      ...
      <description>${escape(t.transcription?.slice(0, 500) || 'Témoignage du mouvement KinkySexy')}</description>
      ...
      <itunes:author>${escape(t.authorName || 'Kinky')}</itunes:author>
...
<rss version="2.0" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:content="http://purl.org/rss/1.0/modules/content/">
  <channel>
    <title>KinkySexy — Témoignages</title>
    <link>${SITE}</link>
    <language>fr-FR</language>
    <copyright>KinkySexy</copyright>
    <itunes:author>KinkySexy</itunes:author>
    ...
    <itunes:owner>
      <itunes:name>KinkySexy</itunes:name>
      <itunes:email>contact@kinky.pixeeplay.com</itunes:email>
    </itunes:owner>
    ...
    <itunes:explicit>false</itunes:explicit>
    <description>Voix LGBT+ et croyantes du monde entier — KinkySexy podcast.</description>
    ${items}
  </channel>
</rss>`;
