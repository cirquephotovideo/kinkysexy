  ...
  try {
    ...
    if (requireApproval && (category as any)?.telegramAlertEnabled !== false) {
      notify({
        ...
        url: `https://kinky.pixeeplay.com/admin/forum?pending=${thread.id}`,
        level: 'info',
        metadata: {
          threadId: thread.id,
          ...
          inlineButtons: [
            ...
            { text: '👁 Voir',      url: `https://kinky.pixeeplay.com/admin/forum?pending=${thread.id}` }
          ]
        }
      }).catch(() => {});
    }
        ...
        if (modResult.shouldHide) {
          ...
          if (isEnabled(cfg, AI_KEYS.modNotifyAdmin)) {
            notify({
              ...
              url: `https://kinky.pixeeplay.com/admin/forum`,
              level: 'warning',
