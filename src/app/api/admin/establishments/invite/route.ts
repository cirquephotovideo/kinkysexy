...
export async function POST(req: NextRequest) {
  ...
  const from = (await prisma.setting.findUnique({ where: { key: 'integrations.resend.from' } }).catch(() => null))?.value || 'Kinky <hello@kinky.pixeeplay.com>';
  const inviteLink = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://kinky.pixeeplay.com'}/admin/pro/onboard?invite=${encodeURIComponent(email)}&name=${encodeURIComponent(name)}&city=${encodeURIComponent(city || '')}`;

  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 24px;">
      <h1 style="color: #d61b80;">✨ Invitation KinkySexy</h1>
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: email,
      subject: `Invitation : rejoins l'annuaire LGBT-friendly de Kinky`,
      html
    })
  });
