...
export async function POST(req: NextRequest) {
  ...
  const prompt = `Tu rédiges une newsletter HEBDO pour KinkySexy (réseau social inclusif religieux LGBT+).
...
- Termine par un CTA clair vers https://kinky.pixeeplay.com`;
  ...
  const fullHtml = wrapHtml(parsed.subject || 'Kinky Newsletter', parsed.preheader || '', parsed.html || '');

  // Crée le draft NewsletterCampaign
  const draft = await prisma.newsletterCampaign.create({
    data: {
      subject: parsed.subject || `Kinky — Semaine du ${weekEnd}`,
...
<h1 style="font-size:22px;color:#d4537e;margin:0 0 16px;">🌈 KinkySexy</h1>
${body}
<hr style="margin:32px 0;border:0;border-top:1px solid #eee;"/>
<p style="font-size:12px;color:#999;text-align:center;">
Tu reçois ce mail car tu es abonné·e à la newsletter Kinky.<br/>
<a href="https://kinky.pixeeplay.com">kinky.pixeeplay.com</a> · <a href="{{unsubscribeUrl}}">Se désabonner</a>
</p>
</div></body></html>`;
}
