...
const BASE = process.env.NEXT_PUBLIC_SITE_URL || 'https://kinky.pixeeplay.com';
      ...
      if (isDelivered) {
        ...
        if (o.email) {
          await sendEmail(
            ...
            `<p>Bonjour ${o.name || ''},</p><p>Bonne nouvelle : ta commande a été <strong>livrée</strong> ! 🎉</p><p>Merci pour ton soutien au mouvement.<br>L'équipe KinkySexy 🌈</p>`
          ).catch(() => {});
        }
        if (o.phone) {
          await sendSMS(o.phone, `✅ Kinky : Ta commande #${o.id.slice(0, 8)} a été livrée. Merci pour ton soutien ! 🌈`).catch(() => {});
        }
