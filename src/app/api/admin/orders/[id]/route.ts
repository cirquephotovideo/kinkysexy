...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (becameShipped) {
    ...
    if (updated.email) {
      ...
      const html = `
        <div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;padding:20px">
          ...
          <p style="color:#666;font-size:14px;margin-top:30px">Merci de soutenir le mouvement 🌈<br>L'équipe KinkySexy</p>
        </div>`;
    ...
    // SMS
    if (updated.phone) {
      const smsBody = `📦 Kinky : Ta commande #${updated.id.slice(0, 8)} est expédiée${updated.trackingNumber ? ` !\nSuivi : ${updated.trackingNumber}\n${updated.trackingUrl || ''}` : '.'}\nDétails : ${followLink}`;
