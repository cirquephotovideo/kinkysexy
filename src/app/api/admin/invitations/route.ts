...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const inviteUrl = `https://kinky.pixeeplay.com/admin/login?invitation=${code}`;
    const html = `
      <div style="font-family:system-ui;max-width:600px;margin:0 auto;padding:24px;background:#fff;color:#222">
        <h1 style="color:#d61b80">🔑 Invitation Kinky Admin</h1>
        <p>Bonjour,</p>
        <p>Tu as été invité·e à rejoindre le back-office <strong>KinkySexy</strong> avec le rôle <strong>${role}</strong>.</p>
        ...
        <p style="font-size:11px;color:#888">
          🌈 KinkySexy — Mouvement interreligieux pour réconcilier foi et diversité.<br/>
          Si tu n'es pas l'expéditeur ou que tu ne reconnais pas cette invitation, ignore-la simplement.
        </p>
      </div>
    `;

    await sendEmail(email, `🔑 Invitation Kinky Admin (${role}) — expire dans ${ttlHours}h`, html, { type: 'admin-notify' }).catch(() => {});
