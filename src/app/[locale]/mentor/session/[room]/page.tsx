...
export const metadata = { title: 'Session mentor — Kinky' };
...
export default async function P({ params }: { params: Promise<{ room: string }> }) {
  ...
  const cleanRoom = `kinky-mentor-${room.replace(/[^a-zA-Z0-9-]/g, '').slice(0, 32)}`;
  ...
  return (
    <main className="container-wide py-6 max-w-5xl">
      ...
      <div className="mt-3 bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-xs text-zinc-300">
        ...
        Partage le lien <code className="bg-zinc-950 px-1 rounded text-[10px]">{`https://kinky.pixeeplay.com/mentor/session/${room}`}</code> avec ton·ta mentor·e/mentee.
        Aucun compte requis.
      </div>
    </main>
  );
