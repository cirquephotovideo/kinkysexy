    ...
    return (
      <main className="container-wide py-12 max-w-2xl">
        <header className="text-center mb-6">
          ...
          <p className="text-zinc-400 text-sm max-w-xl mx-auto">Pour les pays hostiles : ton navigateur ressemble à une <strong>calculatrice ordinaire</strong>. Tape le code secret <strong>{SECRET_PIN}</strong> puis = pour revenir à Kinky.</p>
        </header>
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-sm text-amber-200 mb-4">
          ⚠ <strong>Limite</strong> : ce n'est pas un déguisement complet — l'historique navigateur garde la trace de kinky.pixeeplay.com. Pour vraie discrétion : VPN + navigation privée + <Link href="/voyage-safe" className="underline">checklist voyage</Link>.
        </div>
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4 bg-zinc-100" style={{ background: '#e5e7eb' }}>
      <div className="bg-white rounded-3xl shadow-2xl p-6 w-full max-w-xs">
        ...
        {pinUnlocked && (
          <div className="mt-3 bg-emerald-100 border border-emerald-300 rounded p-2 text-xs text-emerald-800 text-center">🔓 Kinky se charge…</div>
        )}
      </div>
    </main>
  );
