...
function PostEditor({ post, onClose, onSaved }: {
  ...
  async function aiSuggest() {
    ...
    const r = await fetch('/api/ai/text', {
      ...
      body: JSON.stringify({
        prompt: `Rédige un post court (≤280 caractères) pour ${channels.join(', ')} pour le mouvement KinkySexy. Inclusif, lumineux, hashtag #KinkySexy.`,
        system: 'Tu es community manager de KinkySexy.'
      })
    });
    const j = await r.json();
