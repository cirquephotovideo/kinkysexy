...
export function MenuManager({ initial }: { initial: Item[] }) {
  ...
  async function seedDefaults(wipe: boolean) {
    if (wipe && !confirm('Effacer TOUS les éléments du menu actuel et réinitialiser au menu par défaut Kinky ? Cette action est irréversible.')) return;
    if (!wipe && !confirm('Ajouter les éléments manquants du menu par défaut Kinky (Le Message, Argumentaire, Communauté, Agenda, etc.) sans toucher aux items custom existants ? Les doublons par URL seront automatiquement nettoyés.')) return;
  ...
  return (
    <>
      ...
      <div className="bg-blue-500/5 border border-blue-500/20 rounded-lg p-2.5 mb-4 text-[11px] text-blue-200">
        ...
        Si tu vois des doublons "Photos" ou "Boutique", clique <strong>« Seed menu défaut Kinky »</strong> ci-dessus pour les supprimer.
      </div>
