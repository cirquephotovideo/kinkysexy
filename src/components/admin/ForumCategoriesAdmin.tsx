...
export function ForumCategoriesAdmin({ initial }: { initial: Category[] }) {
  ...
  async function seedDefaults() {
    if (!confirm(`Créer ${DEFAULT_CATEGORIES.length} catégories par défaut adaptées à Kinky ?`)) return;
    setBusy('seed');
    try {
    ...
    <section>
      ...
      {items.length === 0 ? (
        <div className="bg-amber-500/5 border border-amber-500/30 rounded-xl p-4 text-amber-200 text-sm">
          ⚠ Aucune catégorie créée. Clique sur <strong>« Seed 6 catégories Kinky »</strong> ci-dessus pour démarrer rapidement,
