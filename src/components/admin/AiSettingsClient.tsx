...
const TASK_LABELS: Record<string, { label: string; tokens: number; description: string }> = {
  ...
  'rag-chat':          { label: '"Demandez à Kinky" RAG',                  tokens: 3000, description: 'Chat avec retrieval sur knowledge base.' },
  ...
  'avatar-realtime':   { label: 'Avatar IA temps-réel',                  tokens: 0,    description: 'Kinky Live.' }
};
...
export function AiSettingsClient() {
  ...
  const [testPrompt, setTestPrompt] = useState('Dis bonjour à la communauté Kinky en 20 mots.');
      ...
      {activeTab === 'tasks' && (
        <section className="space-y-2">
          <p className="text-xs text-zinc-400 mb-3 italic">
            Pour chaque tâche, choisis le provider primaire + les fallbacks. Si le primaire échoue, Kinky essaie automatiquement les fallbacks dans l'ordre.
          </p>
...
# Recommandé : Tailscale Subnet Router</code></pre>
          ...
          <p className="text-xs">LM Studio app Mac → Server tab → Start. Port défaut <code className="bg-zinc-800 px-1 rounded">1234</code>. Permet de swap les modèles depuis la GUI sans toucher au code Kinky.</p>
...
llama-server -m ~/models/qwen2.5-7b-instruct-q5_k_m.gguf -c 8192 --port 8080 --host 0.0.0.0</code></pre>
          ...
          <h4 className="text-sm font-bold text-fuchsia-400 mt-4">9. Stratégie de chaînage recommandée pour Kinky</h4>
