...
export function MonEspaceClient({ user, stats, flags }: Props) {
  ...
  const SHORTCUTS = [
    ...
    { flag: 'kinkyLocal',          href: '/kinky-local',        icon: MapPin,      label: 'Kinky Local',       color: 'cyan' },
      ...
      <header className="flex items-start justify-between flex-wrap gap-3 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-xs mt-1">Ton espace personnel Kinky · membre depuis {user?.createdAt ? new Date(user.createdAt).toLocaleDateString('fr-FR') : '?'}</p>
          </div>
        </div>
