...
export function ThemeApplier() {
  ...
  useEffect(() => {
    ...
    setMusicAllowed(localStorage.getItem('kinky_theme_music') === '1');
  }, []);
  ...
  useEffect(() => {
    ...
    // Custom CSS
    let styleEl = document.getElementById('kinky-theme-custom-css') as HTMLStyleElement;
    if (!styleEl) {
      styleEl = document.createElement('style');
      styleEl.id = 'kinky-theme-custom-css';
  ...
  function toggleMusic() {
    ...
    localStorage.setItem('kinky_theme_music', next ? '1' : '0');
  }
...
function ParticleLayer({ kind, emoji, count, speed, reverse, static: isStatic }: {
  ...
  return (
    <div className="kinky-particles" data-kind={kind}>
      {items.map(p => (
        <span
          key={p.id}
          className={`kinky-particle ${reverse ? 'kinky-particle-up' : 'kinky-particle-down'} ${isStatic ? 'kinky-particle-twinkle' : ''}`}
          style={{
            left: p.left,
...
function RainbowBar() {
  return (
    <div className="kinky-rainbow-bar" />
  );
}

function FireworksLayer() {
  return (
    <div className="kinky-fireworks">
      {Array.from({ length: 5 }, (_, i) => (
        <div key={i} className="kinky-firework" style={{
          left: `${20 + i * 15}%`,
