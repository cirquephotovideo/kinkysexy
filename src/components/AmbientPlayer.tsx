...
export function AmbientPlayer() {
  ...
  useEffect(() => {
    fetch('/api/settings/public?keys=audio.tracks,audio.enabled')
      ...
      .catch(() => {});
    const savedEnabled = localStorage.getItem('kinky_audio_enabled');
    if (savedEnabled === '1') setEnabled(true);
    const savedVol = localStorage.getItem('kinky_audio_volume');
  ...
  function toggle() {
    const next = !enabled;
    setEnabled(next);
    localStorage.setItem('kinky_audio_enabled', next ? '1' : '0');
  ...
  function setVol(v: number) {
    setVolume(v);
    localStorage.setItem('kinky_audio_volume', String(v));
  }
