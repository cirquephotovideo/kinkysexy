...
export function AlarmMode({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    try {
      const contacts = JSON.parse(localStorage.getItem('kinky_emergency_contacts') || '[]');
      if (!Array.isArray(contacts) || contacts.length === 0) return;
      // Géoloc rapide
      navigator.geolocation?.getCurrentPosition((pos) => {
        const mapsLink = `https://www.google.com/maps?q=${pos.coords.latitude},${pos.coords.longitude}`;
        const body = encodeURIComponent(`🚨 URGENCE — j'ai déclenché l'alarme Kinky. Position : ${mapsLink}`);
        ...
        const body = encodeURIComponent('🚨 URGENCE — j\'ai déclenché l\'alarme Kinky. Géoloc indisponible.');
        const firstPhone = contacts[0].phone?.replace(/\s/g, '');
