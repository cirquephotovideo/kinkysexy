...
const NAV: Entry[] = [
  ...
  {
    id: 'connect',
    label: '🌈 Kinky Connect (réseau social)',
    icon: Heart,
    children: [
  ...
  },
  {
    ...
    children: [
      ...
      { href: '/admin/ai/avatar',               label: 'Kinky Live (avatar vidéo)',                icon: Video },
...
const STORAGE_KEY = 'kinky-admin-sidebar-open';
const COLLAPSED_KEY = 'kinky-admin-sidebar-collapsed';
    ...
    >
      <div className={`p-3 border-b border-zinc-800 flex items-center gap-2 ${isVisuallyCollapsed ? 'flex-col py-3' : 'justify-between p-5'}`}>
        ...
        >
          ...
          {!isVisuallyCollapsed && <span className="font-display font-bold">Kinky Admin</span>}
        </Link>
        {!isVisuallyCollapsed && (
          <Link
            href="/"
