...
const STYLES = [
  { id: 'classic',  label: '🎨 Classique',  desc: 'Cœur néon Kinky' },
...
export function ShareCardClient() {
  ...
  const targetUrl = typeof window !== 'undefined' ? window.location.origin : 'https://kinky.pixeeplay.com';
  ...
  // ────────────────────────────────────────────────────────────
  // Composition image (photo OU IA) + texte + logo Kinky + QR sur Canvas
  // ────────────────────────────────────────────────────────────
  async function composeBrandedCard(bgImageUrl: string) {
    ...
    // 8. Logo Kinky (cœur arc-en-ciel + texte) en bas-gauche
  ...
  function triggerDownload(blob: Blob) {
    ...
    a.download = `kinky-${topic}-${Date.now()}.png`;
  ...
  async function shareNative() {
    ...
    try {
      await navigator.share({ title: 'KinkySexy', text, url: fullUrl });
    } catch {}
  }
  ...
  return (
    <main className="container-wide py-12 max-w-4xl">
      <header className="mb-6 text-center">
        ...
        <p className="text-zinc-400 text-sm">Image 1080×1080 prête pour Instagram, TikTok, X, WhatsApp, Telegram… avec logo Kinky et QR code intégré.</p>
      </header>
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-6">
        ...
        <aside className="space-y-4">
          ...
          {style === 'ai' && (
            <div className="bg-gradient-to-br from-fuchsia-500/10 to-violet-500/10 border border-fuchsia-500/30 rounded-lg p-3">
              ...
              <p className="text-[11px] text-zinc-300 mb-3">
                L'IA Gemini Nano Banana génère une affiche unique en respectant les valeurs Kinky : inclusion, foi, amour. <b>Pas de visages identifiables, pas de symboles religieux clivants.</b>
              </p>
              <button
          ...
          <div className="text-[11px] text-zinc-400 bg-zinc-900 border border-zinc-800 rounded-lg p-3 space-y-1.5">
            ...
            <p>Un QR code menant à <code className="bg-black/30 px-1 rounded">kinky.pixeeplay.com</code> est ajouté en bas à droite de chaque carte.</p>
            <div className="flex items-center gap-2 text-fuchsia-300 font-bold pt-2 border-t border-zinc-800">
              <Sparkles size={12} /> Logo Kinky
            </div>
            <p>Le logo cœur arc-en-ciel Kinky est placé en bas à gauche, automatiquement.</p>
          </div>
        </aside>
      </div>
    </main>
